window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			...
			".dshm-list{max-height:168px;overflow-y:auto;overflow-x:hidden;margin-top:8px;border-top:1px solid rgba(255,255,255,0.12);padding-top:5px;scrollbar-width:thin;scrollbar-color:rgba(255,255,255,0.25) transparent}",
		...
		/** Search NetEase Cloud Music through the host proxy. */
		function neteaseSearch(query) {
			return fetch("/dsh-music/netease/search?q=" + encodeURIComponent(query), { cache: "no-store" })
		...
		function MusicPlayer() {
			...
			var doSearch = function () {
				...
				setSearchError(null);
				setResults(null);
				neteaseSearch(query).then(function (data) {
					setSearching(false);
					if (data.error) {
			...
			var addNetease = function (song) {
				run({
					action: "add",
					url: "/dsh-music/netease/stream?id=" + encodeURIComponent(song.id),
											...
											: results.length === 0
												...
												: results.map(function (song) {
													return h("div", {
														className: "dshm-item",
														key: song.id,
														onClick: handleClick(function () { addNetease(song); })
													}, [
